...
previous_states = []
remaining_moves = 1000000000
while remaining_moves > 0:
    for direction in DIRECTIONS:
        tilt_platform(direction)
    if grid in previous_states:
        cycle_length = len(previous_states) - previous_states.index(grid)
        print(f'cycle found: {cycle_length}')
        remaining_moves %= cycle_length
        previous_states.clear()
    previous_states.append(copy.deepcopy(grid))
    remaining_moves -= 1
